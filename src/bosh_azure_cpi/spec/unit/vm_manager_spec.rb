...
describe Bosh::AzureCloud::VMManager do
  ...
  let(:fake_disk_uuid) { 'aa24ad88126f11e6a1483e1d05defe78' }
  ...
  describe "#create" do
    ...
    before do
      ...
      allow(disk_manager).to receive(:delete_disk).
        and_return(nil)
      allow(disk_manager).to receive(:create_ephemeral_disk).
        and_return(disk_name)
      allow(disk_manager).to receive(:get_disk_uuid).
        with(disk_name).
        and_return(fake_disk_uuid)
    end
  ...
  describe "#attach_disk" do
    ...
    it "attaches the disk to an instance" do
      ...
      expect(disk_manager).to receive(:get_data_disk_caching).
        with(disk_name).
        and_return(cache)
      expect(vm_manager.attach_disk(instance_id, disk_name)).to eq("3e1d05defe78")
    end
  end  
